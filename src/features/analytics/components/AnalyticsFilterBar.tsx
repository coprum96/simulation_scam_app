...
export function AnalyticsFilterBar({ value, onChange }: AnalyticsFilterBarProps) {
  return (
    <div className="space-y-4">
      <h2 className="text-sm font-semibold text-slate-800">{ru.analytics.filtersTitle}</h2>

      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs text-slate-500" htmlFor="analytics-date-from">
            {ru.analytics.filterDateFrom}
          </label>
          <input
        ...
        <div>
          <label className="mb-1 block text-xs text-slate-500" htmlFor="analytics-date-to">
            {ru.analytics.filterDateTo}
          </label>
          <input
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs text-slate-500" htmlFor="analytics-scenario">
        ...
        <div>
          <label className="mb-1 block text-xs text-slate-500" htmlFor="analytics-profile">
        ...
        <div className="flex flex-wrap gap-2">
          {DASHBOARD_OUTCOME_FILTER_VALUES.map((option) => (
            <FilterChip
              ...
              label={
                option === FILTER_ALL ? ru.hub.filterAll : replayOutcomeLabel(option)
              }
